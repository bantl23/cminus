...
func (l *Lexer) String() string {
	return fmt.Sprintf("%s %s [%d:%d]", l.Text(), l.FileName(), l.Row(), l.Col())
}
...
func (l *Lexer) GetToken(value string, lval *yySymType) int {
	...
	default:
		lval.yys = yyErrCode
		log.ErrorLog.Printf(">>>>> Error unknown token\n")
	}
	...
	for keepProcessing == true {
		...
		if err == nil {
			...
			if err == io.EOF {
				...
				log.ErrorLog.Printf(">>>>> Error file read %s\n", err)
			}
			keepProcessing = false
		}
	}
